...
describe('useSSEStream', () => {
    ...
    it('dispatches quick_replies event with options array to onQuickReplies handler', async () => {
        const sseEvents = [
            'event: text\ndata: {"content":"reply"}',
            'event: quick_replies\ndata: {"options":[{"title":"Q1","payload":"P1"}]}',
            'event: done\ndata: {"session_id":"sess-123"}',
        ]
        mockedApiFetch.mockResolvedValue(mockResponse(fakeSSEStream(sseEvents)))

        const onQuickReplies = vi.fn()
        const handlers = {
            onText: vi.fn(),
            onProducts: vi.fn(),
            onTip: vi.fn(),
        ...
        expect(onQuickReplies).toHaveBeenCalledWith([{ "title": "Q1", "payload": "P1" }])
    })

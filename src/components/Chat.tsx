...
import type { Message, Conversation, QuickReply, SourceDetail } from '@/types/api';
...
function QuickActionPills({ onSelect }: { onSelect: (text: string) => void }) {
    const pills = [
        { text: 'მცირე ბიზნესი (1%)', prompt: 'რა არის მცირე ბიზნესის სტატუსი და როგორ დავრეგისტრირდე?' },
        { text: 'დეკლარაციის ვადები', prompt: 'როდის არის ყოველთვიური დეკლარაციის ჩაბარების ბოლო ვადა?' },
        { text: 'საშემოსავლო', prompt: 'როგორ იანგარიშება საშემოსავლო გადასახადი?' },
        { text: 'საპენსიო ფონდი', prompt: 'სავალდებულოა თუ არა საპენსიო შენატანი ინდ. მეწარმისთვის?' },
        { text: 'დღგ (VAT)', prompt: 'რა შემთხვევაში ვხდები დღგ-ს გადამხდელი?' },
    ];
...
export default function Chat() {
    ...
    // Single Source of Truth: sources live in message store (nextMsg.sources).
    // activeCitationSources + activeCitationContent track WHICH message's
    // sources the panel is currently showing.
    const [activeCitationSources, setActiveCitationSources] = useState<SourceDetail[]>([]);
    ...
    // ── Derived: only the sources that are actually cited in the active panel's message ──
    const filteredSources = useMemo(() => {
        if (!activeCitationSources.length || !activeCitationContent) return [];
        const cited = parseCitedIds(activeCitationContent);
        if (!cited.size) return [];
        return activeCitationSources.filter((_s, idx) => cited.has(idx + 1));
    }, [activeCitationSources, activeCitationContent]);

    // ── Citation chip click handler (called from ChatResponse via closure) ──
    // Receives the 0-based index + per-message sources/content.
    // Sources come from the message store (nextMsg.sources), never from useState.
    const handleCitationClick = useCallback((allSourceIndex: number, messageSources: SourceDetail[], messageContent: string) => {
        // Set the active panel to this message's sources & content
        setActiveCitationSources(messageSources);
        setActiveCitationContent(messageContent);

        const target = messageSources[allSourceIndex];
        // Recompute filtered for this message inline (memo hasn't updated yet)
        const cited = parseCitedIds(messageContent);
        const msgFiltered = messageSources.filter((_s, idx) => cited.has(idx + 1));
        const filteredIdx = target
            ? msgFiltered.findIndex((s) => s.id === target.id)
            : -1;
        const panelIdx = filteredIdx >= 0 ? filteredIdx : 0;

        if (citationPanelOpen && activeCitationIdx === panelIdx) {
            setCitationPanelOpen(false);
            setActiveCitationIdx(null);
        } else {
            setActiveCitationIdx(panelIdx);
            setCitationPanelOpen(true);
        }
    }, [citationPanelOpen, activeCitationIdx]);

    // ── Citation panel navigation handler (called from CitationPanel itself) ──
    // Receives a pre-filtered index — no remapping needed.
    const handlePanelSourceClick = useCallback((filteredIdx: number) => {
        if (citationPanelOpen && activeCitationIdx === filteredIdx) {
            setCitationPanelOpen(false);
            setActiveCitationIdx(null);
        } else {
            setActiveCitationIdx(filteredIdx);
            setCitationPanelOpen(true);
        }
    }, [citationPanelOpen, activeCitationIdx]);
    ...
    const sendMessage = useCallback(
        async (text: string) => {
            ...
            try {
                ...
                // Reset citation panel state for new message
                setCitationPanelOpen(false);
                setActiveCitationIdx(null);
                setActiveCitationSources([]);
                ...
                await streamMessage({
                    ...
                    handlers: {
                        ...
                        onSources: (sources: SourceDetail[]) => {
                            // Save sources to message store (single source of truth)
                    ...
                    items.push(
                        <div key={msg.id} ref={isLastPair ? lastUserMessageRef : undefined} className="w-full">
                            <ChatResponse
                                ...
                                onCitationClick={(idx) => handleCitationClick(
                                    idx,
                                    nextMsg.sources ?? [],
                                    nextMsg.content ?? ''
                                )}
    ...
    return (
        <div className="flex h-full bg-background overflow-hidden w-full max-w-[1184px]">
            ...
            {/* Citation Panel — receives only sources that are cited in the response */}
            <CitationPanel
                sources={filteredSources}
                isOpen={citationPanelOpen}
                activeIndex={activeCitationIdx}
                onClose={closeCitationPanel}
                onSourceClick={handlePanelSourceClick}
            />

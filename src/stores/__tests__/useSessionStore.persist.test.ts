...
describe('useSessionStore persist', () => {
    ...
    // ─── Partialize: conversations persist (for citation sources) ─────────────

    test('conversations persist to localStorage (citation persistence)', () => {
        useSessionStore.getState().createConversation();
        const stored = JSON.parse(localStorage.getItem('scoop-session') || '{}');
        expect(stored.state.conversations).toBeDefined();
        expect(stored.state.conversations.length).toBe(1);
    });
